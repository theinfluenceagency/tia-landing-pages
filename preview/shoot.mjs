...
for (const [name, vp] of [["desktop", { width: 1440, height: 900 }], ["mobile", { width: 390, height: 844 }]]) {
  ...
  const state = await page.evaluate(() => {
    ...
    return {
      ...
      names: form ? [...form.querySelectorAll("input,select,textarea")].map((e) => e.getAttribute("name")) : null,
      formDataName: form && form.getAttribute("data-name"),
      honeypotHidden: (() => { const e = document.getElementById("middle_name"); return e ? getComputedStyle(e).position === "absolute" : null; })(),
      honeypotLabelHidden: (() => { const l = [...document.querySelectorAll("#quote-form label")].find(x => /Leave this blank/.test(x.textContent)); return l ? getComputedStyle(l).position === "absolute" : null; })(),
      placeholders: form ? [...form.querySelectorAll("input,textarea")].map((e) => e.getAttribute("placeholder")) : null,
